...
class Update extends Component {
		...
		if (this.state.newHistory === this.props.currentAnimal.history[0]) {
			...
			if (imageObjects.length > 0) {
				...
				imageObjects.map(file => {
					uploadImageAsPromise(newCurrent, file).then(
						function(animal) {
							// Update the animal in db
							firebaseRef
								.child(
									this.props.currentAnimal.type +
										this.props.currentAnimal.history[0].status +
										"/" +
										key
								)
								.update(animal)
			...
			if (
				newHistory.lat === this.props.currentAnimal.history[0].lat &&
				newHistory.lng === this.props.currentAnimal.history[0].lng
			) {
				newHistory.lat = null
				newHistory.lng = null
			...
			if (imageObjects.length > 0) {
				...
				imageObjects.map((file, index) => {
					uploadImageAsPromise(newCurrent, file).then(
						function(animal) {
							firebaseRef
								.child("animalsMaster/" + key)
								.set(animal.type + animal.history[date.getTime()].status)
								.then(() => {
									firebaseRef
										.child(
											this.props.currentAnimal.type +
										...
										.then(() => {
											firebaseRef
												.child("animalsWithPics/" + key)
												.set(animal.images[0])
												.then(() => {
													firebaseRef
														.child(
															animal.type +
																animal.history[date.getTime()].status +
																"/" +
																key
														)
														.set(animal)
				...
				firebaseRef
					.child(
						newCurrent.type +
							newCurrent.history[date.getTime()].status +
							"/" +
							key
					)
					.set(newCurrent)
				...
				firebaseRef
					.child("animalsMaster/" + key)
					.set(newCurrent.type + newCurrent.history[date.getTime()].status)
				this.setState({ redirect: true })
			}
		}
	}
	...
	replaceMarkerIcon() {
		...
		newMarker = new google.maps.Marker({
			...
			map,
			icon: require(`../images/mapIcons/${this.state.newHistory.status}${
				this.props.currentAnimal.type
			}Icon.png`)
		})
	...
	findRegion(latLng, google) {
		...
		let region =
			regionName !== undefined ? regionName : "Outside Defined Regions"
		return region
	}
