...
const EventList = (props) => {
    ...
    let Event = props.animalHistory ? props.animalHistory.map((event, i) => {
        ...
        return(
            <tr key={i} onClick={() => {props.handleClick(i + 1, latLng, event.region)}}>
                <td className="detailList__item__id">{number}</td>
                <td className="detailList__item__status">{event.status}</td>
                <td className="detailList__item__date">{new Date(event.date).toLocaleTimeString("en-us", options)}</td>
            </tr>
        )
    }) :
    null
